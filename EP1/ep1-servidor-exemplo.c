...
int main (int argc, char **argv) {
   ...
   ssize_t  n;
	/* Estrutura de dados para impressao de arquivos no diretorio
	 * do servidor */
	DIR *d;
	struct dirent *dir;
	...
	if (argc != 2) {
      ...
      fprintf(stderr,"Vai rodar um servidor FTP na porta <Porta> TCP\n");
		exit(1);
	}
    ...
    * socket passivo.NÃO É A MESMA COISA QUE O MODO PASSIVO DO FTP.
		* Para isto é necessário chamar a função listen
	...
	for (;;) {
      ...
      if ( (childpid = fork()) == 0) {
         ...
         /* ========================================================= */
				 ...
				 write(connfd, "220 GyroFTP Server \r\n", 21);
				 ...
				 write(connfd, "331 Requesting password \r\n", 26);
				 while ((n=read(connfd, recvline, MAXLINE)) > 0) {
					 printf("[Cliente conectado no processo filho %d enviou:] ",getpid());
					 if ((fputs(recvline,stdout)) == EOF) {
							perror("fputs :( \n");
				 ...
				 write(connfd, "230 User logged in \r\n", 21);
         while ((n=read(connfd, recvline, MAXLINE)) > 0) {
					 	/* Faz-se esta instrucao para determinar o fim da string, para o fputs*/
            recvline[n]=0;
            printf("[Cliente conectado no processo filho %d enviou:] ",getpid());
						if ((fputs(recvline,stdout)) == EOF) {
               perror("fputs :( \n");
               exit(6);
						...
						if (strncmp(recvline, "LIST", 4) == 0) {
							d = opendir(".");
							if (d) {
								write(connfd, "200 Command successful\n", 23);
								while ((dir = readdir(d)) != NULL) {
									write(connfd, dir->d_name, strlen(dir->d_name));
									write(connfd, "\n", 1);
								}
								closedir(d);
								write(connfd, "226 Transfer complete\n", 22);
							}
						}
						else if (strncmp(recvline, "QUIT", 4) == 0) {
							write(connfd, "221 See ya!\n", 12);
							break;
						}
						else {
						...
						/* Este fputs é como o servidor imprime para si mesmo
						if ((fputs(recvline,stdout)) == EOF) {
               perror("fputs :( \n");
               exit(6);
            }
						Utilizamos o write para enviar respostas para o cliente
            write(connfd, "batata\n", 7); */
